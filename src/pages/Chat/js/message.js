    ...
    if (!node?.getAttribute('data-format')) {
        for (const reg in ExgType) {
            if (ExgType[reg].test(node.innerHTML)) {
                node.innerHTML = node.innerHTML.trim().replace(ExgType[reg], (match, p1, p2, p3,p4) => {
                    switch (reg) {
                        case 'img':
                            if (p1){ 
                        ...
                        case 'md_code':
                            if (!p2) return `${match}`
                            return `<pre><code class="language-plaintext">${p2.replace(new RegExp(newLine, 'g'), '\n').replace(/\</g, leftArrow).replace(/\>/g, rightArrow)}</code></pre>`;
                        case 'code':
                            if (!p2) return `<pre><code class="language-plaintext">${p4}</code></pre>`
                            if (!p4) return `<code class="language-${p2}">代码段</code>`
                            return `<pre><code class="language-${p2}">${p4.replace(new RegExp(newLine, 'g'), '\n').replace(/\</g, leftArrow).replace(/\>/g, rightArrow)}</code></pre>`;
                        case 'color':
                            if (!p2) return `${match}`
        ...
        node.innerHTML = filterMessage(node.textContent);
...
export { formatMessage,formatEveryMessage, removeSlash }